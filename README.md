...
12. `CaptchaSolver` 会针对每一个 challenge 独立生成 `template-mask`、`template-content`、`image` 三类主候选，并按方法优先级与置信度排序后只提交当前 challenge 的最佳横坐标；只有当前三类主候选全部失效时才会启用 `estimate` 兜底，避免固定默认坐标主导整个重试链路。由于上游首次失败后会让验证码过期，同一验证码仍然只提交一次，失败后重新获取新的 challenge 并重新完整识别
...
11. `src/Captcha/CaptchaSolver.php`
    验证码识别核心模块，负责按单个 challenge 组织模板匹配、图像连通域检测和近似兜底检测，输出带置信度的候选结果，调用校验接口，并在校验失败时对新 challenge 重新识别。
...
18. 验证码求解不再由单一的 `image` 路径主导，而是同时保留 `template-mask`、`template-content`、`image` 三类主候选来源；模板候选优先从“形状遮罩”和“内容差异”两个方向给出坐标，并按方法优先级和置信度排序，用来打破固定退化到单一左值的问题。
19. `estimate` 兜底估算不再作为常驻主候选参与排序，只在模板和图像检测全部失效时才启用，减少固定默认坐标绑架整条求解链路的问题。
20. `CaptchaSolver` 对同一个验证码 uuid 只提交一次 `checkImage`；如果失败，会重新获取验证码并对新的 challenge 重新完整识别，而不是机械轮换上一轮候选偏移，避免触发“校验图片信息过期”后仍在错误中心附近循环。
...
5. 验证码候选横坐标会优先来自模板匹配和图像连通域检测，并按照方法优先级与置信度做显式排序；`estimate` 只作为最后兜底。单个验证码只提交一个坐标，失败后重新获取验证码并对新 challenge 重新识别，避免上游把同一 uuid 的后续尝试全部判定为过期。
...
8. 当前测试已覆盖域名规范化、环境预检行为、缓存版本、响应字段完整性、配置边界、`404` 可缓存标志、列表候选项选择、标识符字段变体、列表详情兜底，以及验证码候选排序和去重规则，但仍不足以替代完整的并发、锁竞争和真实上游集成测试。
...
验证码相关 debug 日志会记录检测方法、候选坐标、候选置信度、当前验证码尝试序号、每次 `checkImage` 拒绝时的上游返回码和消息，用于排查模板路径失效、图像路径误检、主候选排序异常和验证码窗口过期等问题。
